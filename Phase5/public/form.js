...
function buildValidationMessage(errors) {
  if (!Array.isArray(errors) || errors.length === 0) {
    return "Validation failed. Please check your input fields and try again.";
  }
  ...
  return `Oops, some fields look to have invalid data. Please fix the error below and try again:\n\n${lines.join("\n")}`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        const msg = `Oh no!\n\nThe resource name "${payload.resourceName}" already exists. Please choose a unique name or check the existing resources.`;
    ...
    const msg = `🎉 Success! Your resource "${body?.data?.name}" was created at ➡️ ${createdAt} perfectly.\nIt is now saved in the database (ID: ${body?.data?.id}).`

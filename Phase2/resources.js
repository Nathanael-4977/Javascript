...
function attachResourceFormValidation(nameInput) {
  const descInput = document.getElementById('resourceDescription');
  const update = () => {
    const nameValue = nameInput.value;
    const descValue = descInput.value;
    const isNameValid = isResourceNameValid(nameValue);
    const isDescValid = isResourceDescValid(descValue);
    if (nameValue.length > 0) {
        setInputVisualState(nameInput, isNameValid ? "valid" : "invalid");
    } else {
        setInputVisualState(nameInput, "neutral");
    }

    if (descValue.length > 0) {
        setInputVisualState(descInput, isDescValid ? "valid" : "invalid");
    } else {
        setInputVisualState(descInput, "neutral");
    }
...
attachResourceFormValidation(resourceNameInput);
